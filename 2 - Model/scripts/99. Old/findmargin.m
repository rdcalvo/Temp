function s = findmargin(mag,phase,w)
...
%ic = find((logmag(:,1:nf-1)+3-logmag(:,1)) .* (logmag(:,2:nf)+3-logmag(:,1))  <= 0 & (logmag(:,1:nf-1)+3-logmag(:,1))~=(logmag(:,2:nf)+3-logmag(:,1)));
%ic = find((logmag(:,1:nf-1)) .* (logmag(:,2:nf))  <= -3 );

ic = find(20*log10(mag) < -3,1);
if isempty(ic)
    Wcp = zeros(1,0);
elseif ic == 1
    Wcp = interp1([20*log10(mag(ic)) 20*log10(mag(ic+1))],[w(ic) w(ic+1)],-3);
else
    Wcp = interp1([20*log10(mag(ic-1)) 20*log10(mag(ic))],[w(ic-1) w(ic)],-3);
end
