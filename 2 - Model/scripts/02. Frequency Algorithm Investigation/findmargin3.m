function [Gm,Wcg,Pm,Wcp,Bw,Bc] = findmargin3(mag,phase,w)
...
%------------------------------------
% Bandwidth (DC - 0.5dB gain crossing)
%------------------------------------

% Find points where magnitude crosses 0 db

GainDC = 20*log10(mag(1));
ic = find(20*log10(mag) < GainDC-0.5,1);
if isempty(ic)
    Bc = 'Not found';
elseif ic == 1
    Bc = interp1([20*log10(mag(ic)) 20*log10(mag(ic+1))],[w(ic) w(ic+1)],GainDC-0.5);
else
    Bc = interp1([20*log10(mag(ic-1)) 20*log10(mag(ic))],[w(ic-1) w(ic)],GainDC-0.5);
end
